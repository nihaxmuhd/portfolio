...
import { CheckCircle2, Edit, ExternalLink, Image, Plus, Sparkles, Trash2, X } from 'lucide-react';
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map(tag => (
            <span
              key={tag}
              className="rounded-full border border-violet-500/15 bg-violet-500/10 px-3 py-1 text-[11px] font-semibold text-violet-600 dark:text-violet-300"
            >
              {tag}
            </span>
          ))}
...
function ProjectForm({ editingId, initial, onSubmit, onCancel }) {
  ...
  const submit = e => {
    ...
    onSubmit({
      ...
      image_url: imgUrl.trim() || `https://picsum.photos/id/${Math.floor(Math.random() * 90) + 10}/900/560`,
  ...
  return (
    <form onSubmit={submit} className="glass-card mx-auto mb-10 max-w-4xl rounded-[2rem] p-6 sm:p-8">
      ...
      <div className="mt-4">
        ...
        <div className="relative">
          <Image className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          <input type="url" value={imgUrl} onChange={e => setImgUrl(e.target.value)} className="ui-input pl-11" placeholder="https://images.unsplash.com/..." />
        </div>
      </div>
...
export default function Projects({ isAdmin }) {
  ...
  const handleFormSubmit = async payload => {
    try {
      if (editingId) {
        const updated = await api.updateProject(editingId, payload);
