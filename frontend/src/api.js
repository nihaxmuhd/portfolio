...
const handleResponse = async response => {
  ...
  if (!response.ok) {
    const message = typeof data === 'string'
      ? data
      : data.detail || data.non_field_errors?.[0] || 'Something went wrong';
...
const buildProjectFormData = projectData => {
  ...
  ['title', 'description', 'tech_stack', 'order', 'project_progress', 'status'].forEach(field => {
  ...
  ['github_url', 'live_url', 'image_url'].forEach(field => {
    if (projectData[field] !== undefined && projectData[field] !== null) {
      formData.append(field, projectData[field]);
    }
  });

  if (Array.isArray(projectData.existing_image_ids)) {
    projectData.existing_image_ids.forEach(id => {
      formData.append('existing_image_ids', id);
    });
  }

  if (projectData.sync_existing_images) {
    formData.append('sync_existing_images', '1');
  }

  if (Array.isArray(projectData.new_images)) {
    projectData.new_images.forEach(file => {
